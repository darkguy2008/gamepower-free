...
var GPScreen = Class.extend({
	...
	recreate: function() {
		...
		// TODO: Mouse show/hide if graph != 0
		// $(this._canvas).css('cursor', 'none');
	...
	refresh: function() {
		...
		var scale = this._propScale([this.options.size[0], this.options.size[1]], [$(window).width(), $(window).height()]);
        ...
        if($(window).height() > this._canvas.height)
        {
            $(this._canvas).css({
                "position" : "absolute",
                "top" : ($(window).height()/2 - this._canvas.height/2) + "px",
                "left" : "0px",
                'width' : "100%"
            });
        }
